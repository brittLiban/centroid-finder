...
I, Kaalid understand to not use AI for generating implementations of the classes and that I can mindfully only use AI to help me make test cases.

DO NOT MIX HUMAN AND AI COMMITS Bahram.
...
```
Perceptual Method run steps: 

git switch wave7-enhancement 

To run perceptual method to generate binarized image use - javac -cp lib/junit-platform-console-standalone-1.12.0.jar src/*.java && java -cp src ImageSummaryApp sampleInput/squares.jpg FFA200 55
...
The Perceptual color distance outputs the image using yellow/white color to make it look almost exactly like the original image.
